...
def create_images_and_mask(args):
    ...
    for image_id in tqdm(range(hm_img)): # idx changed to image_id      
        ...
        C = np.random.randint(Nx-2, size=(num,2))
        ...
        image = part_img
        #mask = np.maximum(fiber_mask, part_mask) # all the fibers and perticles are labelled
        mask = fiber_mask # only fibers are labelled
        #mask = part_mask # only particles are labelled
